...
export function PrdEditor({
  ...
  prd: Tables<'prds'> | null
  workshopId: string
  canEdit: boolean
  onSaved(prd: Tables<'prds'>): void
}) {
  ...
  async function save() {
    setIsSaving(true)
    const response = await fetch(`/api/prd/${currentPrd.id}`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ workshop_id: workshopId, content: draft }),
    })
    const payload = await response.json().catch(() => null)
    setIsSaving(false)

    if (!response.ok) {
      toast.error(payload?.error?.message ?? 'PRD를 저장하지 못했습니다.')
      return
    }
