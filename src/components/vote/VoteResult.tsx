...
export function VoteResult({ visible, results }: { visible: boolean; results: VoteResultRow[] }) {
  if (!visible) {
    return (
      <div className="rounded-lg border border-dashed border-neutral-800 bg-neutral-900/50 p-6 text-sm text-neutral-300">
  ...
  return (
    <section className="rounded-lg border border-neutral-800 bg-neutral-900/70 p-4">
      <h3 className="mb-4 text-sm font-semibold text-white">투표 결과</h3>
      <div className="space-y-3">
        {results.map((result) => (
          <div key={result.cluster_id ?? result.note_id} className="grid gap-2">
            <div className="flex items-center justify-between gap-3 text-sm">
              <span className="min-w-0 truncate text-neutral-200">{result.target_name}</span>
