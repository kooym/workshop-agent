...
import { processLanePatchSchema } from '@/lib/api/validators'
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  return withAuth(
    ...
    async (_request, { service, participant }) => {
      ...
      const parsed = processLanePatchSchema.safeParse(body)
      if (!parsed.success) {
        return error(API_ERROR_CODES.VALIDATION_ERROR, 'Swimlane 수정 정보를 확인해주세요.', 400)
      }
      ...
      if (!lane) {
        return error(API_ERROR_CODES.NOT_FOUND, 'Swimlane을 찾을 수 없습니다.', 404)
      }
