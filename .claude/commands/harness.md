...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 `npm run build && npm test` 같은 실제 실행 가능한 검증 커맨드를 포함한다. 산문형 AC ("에러가 없어야 한다")는 금지.
...
8. **문서 우선** — 기획/아키텍처 변경이 필요한 step은 먼저 `/docs/` 문서를 업데이트한 후 코드를 수정하도록 명시한다. 주요 dependency 변경 시 `FOUNDATION_ASSESSMENT.md` 또는 `ADR.md` 업데이트 의무화.
9. **TDD 강제** — 각 step의 작업 섹션에 "테스트를 먼저 작성한다"를 명시한다. CLAUDE.md CRITICAL 규칙 #9 준수.
...
**테스트 우선 (TDD)**: 아래 구현을 시작하기 전에 먼저 테스트 케이스를 작성하라.
...
docker build -t workshop-agent .  # Docker 이미지 빌드 성공
```

모든 검사가 통과해야 step을 completed로 마킹할 수 있다.
